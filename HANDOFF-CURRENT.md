...
3. **Сторож квоты Go** — **решение пользователя: пока НЕ нужен** (2026-09-18). Отдельного
   cron/watchdog нет и не делаем; откат остаётся по факту ошибки (релей auto + fallback-цепочка
   Hermes) и вручную через `studio-model-switch.sh`. Не поднимать без новой просьбы.
