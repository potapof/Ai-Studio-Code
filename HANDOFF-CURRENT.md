...
1. **Коммит/пуш изменений** `~/studio` (скрипты `zen-go-relay.py`, `studio-model-switch.sh`,
   `start-holix-gateway.sh`, `studio-readiness.sh`, `docs/16-design-studio.md`) — по умолчанию не
   коммитил. Сказать «коммить» — закоммичу и запушу в Ai-Studio-Code.
2. **Сторож квоты Go** (не делал): сейчас откат срабатывает по факту ошибки у клиентов релея и
...
3. OpenCode CLI — **решение пользователя: не ставить, вопрос закрыт** (не поднимать).
4. Ключ подписки — **закрыто**: перенесён в `~/studio/.env` (+ `~/.hermes/.env` для Hermes),
